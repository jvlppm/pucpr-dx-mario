...
struct Camera::private_implementation {
    ...
    void begin(IDirect3DDevice9* device, shared_ptr<Scene> scene) {
        ...
        else { // Post processing enabled
            HR(device->GetRenderTarget(0, &defaultRenderTarget));
    ...
    void end(IDirect3DDevice9* device) {
        ...
        if (defaultRenderTarget) {
            HR(device->SetRenderTarget(0, defaultRenderTarget));
            defaultRenderTarget = nullptr;
        }
    }
    ...
    void draw(IDirect3DDevice9* device) {
        ...
        shader->begin(device);
        shader->setTexture(renderTexture);
...
Camera::Camera(IDirect3DDevice9* device, const std::string& shader)
    : Camera()
{
...
shared_ptr<Camera> Camera::setShader(shared_ptr<IShader> shader) {
